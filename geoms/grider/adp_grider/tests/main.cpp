...
int main(int argc, char **argv)
{
	...
	std::string file_path_1 = cwd_path_str + "/adp_grid_data_a_" + std::to_string(num_partitions);	
	...
	std::string file_path_2 = cwd_path_str + "/adp_grid_data_b_" + std::to_string(num_partitions);	

	if (-1 == access(file_path_2.c_str(), F_OK)){
			mkdir(file_path_2.c_str(),S_IRWXU);
	}
	
	std::string grid_path = cwd_path_str+"/adp_grid_wkt_file_"+ std::to_string(num_partitions);
